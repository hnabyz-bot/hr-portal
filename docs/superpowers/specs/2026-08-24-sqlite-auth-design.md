...
- "본부장이 보는 범위"는 **자기 본부 소속 전원**이다 (본부 직속 + 하위 팀 전원). 전사가 아니다.
- "자기 본부"는 `departments.leader_user_id = 본인` 이면서 `level = 'DIVISION'`인 부서다.
...
SQLite에는 정확한 소수 자료형이 없다. `REAL`은 부동소수점이라 `33.33 + 33.33 + 33.34`가 정확히 `100.00`이 되지 않는다. **가중치 합계 100 검증이 이 오차로 깨진다.**
