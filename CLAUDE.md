...
- `main`/`release` 직접 push 금지 — **2026-08-26부터 브랜치 보호 규칙으로 실제로 차단된다.** `main`은 PR 필수 + CI 4종 통과 필수(승인자 0명, 관리자 우회 금지), `release`는 강제 push·삭제만 금지(CI가 직접 갱신해야 하므로).
...
이슈 #11이 제안한 항목은 2026-08-26에 대부분 완료됐다 — [CONTRIBUTING.md](CONTRIBUTING.md)·[DEPLOY.md](DEPLOY.md) 신설(#15), PR 템플릿(#14), 브랜치 보호 규칙(#12), CI 머지 필수화(#16), 병합 브랜치 정리(#17). **아직 안 한 것은 이슈 템플릿 하나뿐**이며, 이슈 수가 적어 오히려 진입 장벽이 될 수 있다고 판단해 만들지 않았다(필요하면 요청).

수정 대상은 대부분 `public/index.html` (단일 파일, 인라인 CSS/JS). `docker-compose.yml`/`nginx.conf`/`backend/`도 경비 산출기·규정검색 백엔드가 생기면서 실제로 쓰이는 파일이다.

## 인사담당자 통지 규칙 (2026-08-26, 사용자 지시)

협업자 `irene170304-netizen`이 **비개발자이자 인사담당자**다. 인사 규칙은 이 사람이 정한다. 내가 판단해서 만들면 안 되는 영역이다.

**매번 판단하지 말고 아래를 기계적으로 적용한다.** 판단에 맡기면 빠뜨린다.

### 언제 통지하는가

| 통지한다 | 통지하지 않는다 |
|---|---|
| **인사 규칙 결정이 필요할 때** (권한 범위, 등급 배분, 결재선, 보관기간 등) | 순수 기술 작업 (리팩터링, CI, 스키마 변환, 의존성) |
| **그가 쓴 문서·PR의 내용을 고치거나 뒤집을 때** | 내부 구현 선택 (자료구조, 파일 배치) |
| **그가 해야 할 작업이 생겼을 때** (서버 파일 전달, 값 확인 등) | 이미 확정된 규칙을 그대로 구현할 때 |
| **그가 만든 것에서 결함·오류를 찾았을 때** | 문서 오탈자 수정 |

애매하면 통지한다. **통지 비용은 낮고, 안 해서 잘못 만든 비용은 높다.**

### 어떻게 쓰는가

- **개발 용어를 쓰지 않는다.** 꼭 필요하면 비유로 풀어 쓴다 (예: 컨테이너 → "프로그램을 담아 돌리는 상자").
- 항목마다 **① 지금 무슨 상황인가 ② 선택지 ③ 판단에 참고할 점 ④ 잘못 정하면 무슨 일이 생기는가** 순서로 쓴다.
- **"모르겠다고 답해도 된다"**고 명시한다. 모르면 답을 안 하게 되고, 그러면 진행이 멈춘다.
- **"한 번에 다 답하지 않아도 된다"**고 명시한다.
- **답이 없으면 어떤 기본값으로 진행하는지 미리 밝힌다.** 개발이 답을 기다리며 멈추면 안 된다.
- 나중에 바꾸기 비싼 항목은 **왜 비싼지와 함께 먼저 봐달라고** 표시한다.
- 사내 IP·호스트명·시크릿은 쓰지 않는다 (Public 저장소).

### 어떻게 알리는가 — 메일까지 보낸다

**이슈만 만들어두면 안 본다.** 아래를 반드시 같이 한다.

1. GitHub 이슈 생성
2. **담당자로 지정** (`assignees`) → GitHub이 알림 메일 자동 발송
3. **본문에 `@irene170304-netizen` 멘션** → 알림 메일 한 통 더
4. 결정이 중요하면 **README에서 링크**한다 — 문서를 만들어도 README에서 닿지 않으면 아무도 못 찾는다

**메일 주소는 필요 없다.** GitHub 계정에 등록된 주소로 나가며, **받은 메일에 그대로 답장하면 이슈 댓글로 올라간다.** 이 점을 본문에 안내한다.

며칠 반응이 없으면 Gmail 초안(`create_draft`)으로 별도 안내를 만들고 **발송은 저장소 소유자가 확인 후 누른다.** 내가 임의로 발송하지 않는다.

### 확인된 실행 경로 (2026-08-26 점검)

| 경로 | 상태 |
|---|---|
| GitHub 담당자 지정·멘션 → 알림 메일 | ✅ 동작 확인 (이슈 #32) |
| Gmail 초안 작성 (MCP) | ✅ 연결 확인. 발송은 사람이 |
| 서버에서 직접 SMTP 발송 | 포트는 열려 있으나 앱 비밀번호 필요. 자동 발송이 반복될 때만 |
| 로컬 메일 도구 (`sendmail`/`mail`/`postfix`) | 설치되어 있지 않음 |
